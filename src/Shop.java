...
public class Shop {
    public static void main(String[] args) {
        ...
        for (Clothing item : items) {
            if(item.getTamanio().equals("L")){
                cantidad++;
                promedio+=item.getPrecio();}
            //System.out.println("Descripción: " + item.getDescripcion() + ", " + "Precio: " + item.getPrecio() + ", " + "Tamanio: " + item.getTamanio());
        }
        try {
            promedio= promedio/ cantidad;
            System.out.println("precio promedio: "+ promedio + "cantidad: "+ cantidad);
        }catch (ArithmeticException e){
            System.out.println("No dividas x cero panflin");
        }
